    ...
    <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 48px' }}>
      ...
      <section style={{ marginBottom: '48px', paddingBottom: '48px', borderBottom: '1px solid var(--border)' }}>
        ...
        <h1 style={{ fontSize: 'clamp(32px, 5vw, 56px)', fontWeight: 800, lineHeight: 1.1, marginBottom: '16px' }}>
          ...
          <span style={{ color: 'var(--accent)' }}>for Vue 3</span> (2026)
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '18px', maxWidth: '560px', marginBottom: '16px' }}>
          All {vueIcons.length} icon libraries below have official Vue 3 support with dedicated packages, TypeScript definitions, and tree-shaking.
        </p>
      ...
      <section style={{ marginBottom: '48px', paddingBottom: '48px', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '28px' }}>
          <p style={{ color: 'var(--text-muted)', fontSize: '15px', lineHeight: 1.8, marginBottom: '12px' }}>
            For most Vue 3 projects, <span style={{ color: 'var(--text)', fontWeight: 700 }}>Lucide Vue Next</span> is the top recommendation. Install via <code style={{ fontFamily: 'JetBrains Mono, monospace', color: 'var(--green)', fontSize: '13px' }}>npm install lucide-vue-next</code>. It is actively maintained, tree-shakable, and ships with TypeScript definitions.
          </p>
          <p style={{ color: 'var(--text-muted)', fontSize: '15px', lineHeight: 1.8 }}>
            If you need more than 1,400 icons, <span style={{ color: 'var(--text)', fontWeight: 700 }}>Tabler Icons Vue</span> offers 5,500+ icons with the same developer experience.
          </p>
        </div>
      </section>
      ...
      <section style={{ marginBottom: '48px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
          {[['lucide-icons', 'heroicons'], ['lucide-icons', 'tabler-icons'], ['tabler-icons', 'remix-icon'], ['phosphor-icons', 'lucide-icons']].map(([a, b]) => (
