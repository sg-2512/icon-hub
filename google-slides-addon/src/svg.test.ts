...
describe("SVG safety", () => {
  it("removes active content and external references", () => {
    const safe = sanitizeSvg('<svg xmlns="http://www.w3.org/2000/svg" onload="alert(1)"><script>alert(1)</script><image href="https://bad.example/a.png"/><path style="fill:red" fill="url(https://bad.example/p)" stroke="#000"/></svg>');
    expect(safe).not.toMatch(/script|onload|image|style=|bad\.example/i);
    expect(safe).toContain('stroke="#000"');
  });

  it("recolors paint and supplies accessible SVG metadata", () => {
  ...
  it("accepts only HTTPS asset URLs and six-digit colors", () => {
